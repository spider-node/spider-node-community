...
import cn.spider.framework.flow.container.task.impl.AbilityTaskServiceWrapper;
import cn.spider.framework.flow.container.task.impl.TaskComponentProxy;
import cn.spider.framework.flow.container.task.impl.TaskComponentRegisterWrapper;
import cn.spider.framework.flow.exception.ExceptionEnum;
import cn.spider.framework.flow.resource.service.ServiceNodeResource;
import cn.spider.framework.flow.resource.service.ServiceNodeResourceItem;
...
public abstract class TaskComponentRepository implements TaskContainer {
    ...
    protected void doInit(Class<?> targetClass, String taskComponentName, boolean scanSuper) {
        ...
        TaskComponentProxy targetObj = new TaskComponentProxy(taskComponentName);
    ...
    public void unLoader(Class<?> targetClass) {
        ...
        taskComponentWrapperMap.remove(taskComponentName);
        Method[] taskServiceMethods = MethodUtils.getMethodsWithAnnotation(targetClass, TaskService.class, false, false);
        List<Method> taskServiceMethodList = filterTaskServiceMethods(taskServiceMethods, targetClass, taskComponent.scanSuper());
        if (CollectionUtils.isEmpty(taskServiceMethodList)) {
            return;
        }
        taskServiceMethodList.stream().forEach(method->{
            TaskService annotation = method.getAnnotation(TaskService.class);
            String taskServiceName = StringUtils.isBlank(annotation.name()) ? method.getName() : annotation.name();
            ServiceNodeResourceItem serviceNodeResource = new ServiceNodeResourceItem(taskComponentName, taskServiceName, annotation.ability(), annotation.desc());
            taskInstructResourceMap.remove(taskServiceName);
            registeredServiceNodeResource.remove(serviceNodeResource);
        });
